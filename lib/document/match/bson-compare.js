...
exports.compareBy = function(valueA, valueB, fields) {
  return Object.keys(fields)
    .reduce(function(result, field){
      ...
      const compareResult = exports.compare(fieldA, fieldB);
      return direction === -1 ? compareResult * -1 : compareResult;
    }, 0);
};
...
exports.isNull = function(value) {
  return (
    value === exports.FLAG_NON_EXISTENT ||
...
exports.compareObject = function(valueA, valueB) {
  ...
  const valuesB = keysB.map(function(key){ return valueB[key]; });
...
exports['SortOrder'].forEach(function(comparison) {
  const found = Object.keys(exports)
    .find(function(key) {
      return exports[key] === comparison;
    });
  if(!found) {
    return;
  }
  comparison.name = found;
});
