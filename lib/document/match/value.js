...
exports.handleOperatorFilter = function(match, document, filter, filterKey, filterValue, documentValues) {
  ...
  const results = filterKeys
    .filter(function(key){
      ...
      const found = documentValues.filter(function(documentValue){
        // $eq and $ne handle arrays
        if(BSONCompare.isArray(documentValue) && (
          key !== '$eq' &&
          key !== '$ne'
        )) {
          return exports['$elemMatch'](match, filterValue, filterValue[key], documentValue, filterKey, document);
        }
        return exports[key](match, filterValue, filterValue[key], documentValue, filterKey, document);
      });
      // Can't use find because it may return the document value false or null
      return !!found.length;
    });
...
exports['$eq'] = function(match, filterValue, valueA, valueB) {
  if(!BSONCompare.isArray(valueA) && BSONCompare.isArray(valueB)) {
    return exports['$elemMatch'](match, filterValue, valueA, valueB);
  }
...
exports['$gt'] = function(match, filterValue, valueA, valueB) {
  // If valueA (filter) is larger than valueB (document) than compare will be 1
...
exports['$lt'] = function(match, filterValue, valueA, valueB) {
  // If valueA (filter) is smaller than valueB (document) than compare will be -1
  return BSONCompare.compare(valueA, valueB) > 0;
};
...
exports['$exists'] = function(match, filterValue, exists, value) {
  const valueExists = value !== Flags.NON_EXISTENT;
  return exists ? valueExists : !valueExists;
};

exports['$type'] = function(match, filterValue, type, value) {
  return value !== Flags.NON_EXISTENT && BSONCompare.isType(type, value);
};
...
exports['$all'] = function(match, filterValue, all, value){
  if(!BSONCompare.isArray(all) || !BSONCompare.isArray(value)) {
    return false;
  }
  const found = Array.prototype.filter.call(all, function(element) {
    return exports.isMatch(match, { value: value }, { value: element }, 'value');
  });
  return found.length === all.length;
};
...
exports['$elemMatch'] = function(match, filterValue, elemMatch, value){
  ...
  return Array.prototype.find.call(value, function(element) {
    return exports.isMatch(match, { value: element }, { value: elemMatch }, 'value');
  });
};
...
exports['$size'] = function(match, filterValue, size, value){
  ...
  //
  if(!BSONCompare.isLong(value)) {
    return false;
  }
  size = BSONCompare.getLong(value);
  if(isNaN(size)) {
    return false;
  }
