...
exports.FLAG_NON_EXISTENT = Symbol('FLAG_NON_EXISTENT');
...
exports.getFieldFromPlainObject = function(result){
  ...
  if(!objectResult.object.hasOwnProperty(result.fieldDetails.current)){
    return result;
  }
  if(!result.fieldDetails.next){
...
exports.getFieldFromArray = function(result) {
  ...
  if(result.fieldDetails.current === "$"){
    ...
    objectResult.field = 0;
    objectResult.values = [objectResult.object[objectResult.field]];
    objectResult.exists = objectResult.object.length > 0;
    return result;
  }
  ...
  const next = objectResult.object
