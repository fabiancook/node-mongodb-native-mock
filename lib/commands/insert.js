...
exports.insertDocument = function(bson, db, cmd, document){
  return exports.checkDuplicate(bson, db, cmd, document)
    .then(function(){
      return db.put(bson.serialize(document._id), bson.serialize(document));
    });
};
...
exports.checkDuplicate = function(bson, db, cmd, document) {
  ...
  db.get(bson.serialize(document._id), deferred.makeNodeResolver());

  return deferred.promise
    .then(function(){
      var msg = "insertDocument :: caused by :: 11000 E11000 duplicate key error index: " + levelDb.mongoDb.s.databaseName + "." + cmd.insert + ".$_id_  dup key: { : " + document_id.toString( ) + " }";
